  ...
  handleCodapNotification(iNotification) {
    ...
    if (tTask) {
      ...
      */
      ...
      handleAttributeChange = function () {
        ...
        codapInterface.sendRequest({
          ...
          if (maxAttrsFound = 1) {
            ...
            if (attributeName === 'Total Count of Medical Doctors' && attributeAxis === 'horizontal') this.handleAccomplishment('AddDoctors');else if (attributeName === 'Internet Users' && attributeAxis === 'horizontal') this.handleAccomplishment('AddInternetUsers');
          };
        }.bind(this));
      }.bind(this),
          handleTitleChange = function () {
        //Get ID of notification component
        console.log('iNotification.resource', iNotification.resource);
        codapInterface.sendRequest({
          action: 'get',
          resource: iNotification.resource
        }).then(function (iResult) {
          console.log(iResult);
          console.log(iResult.values.type);
          if (iResult.values.type === 'graph') {
            console.log('Made it here.');
            this.handleAccomplishment('ChangeGraphTitle');
          }
        }.bind(this));
      }.bind(this),
      ...
      /*if (tGraphRequestList.length > 0) {
        codapInterface.sendRequest(tGraphRequestList).then(function (iResults) {
          let tNotificationComponentName = iNotification.values[0].name;
          let wasGraph = iResults.some(function (iResult) {
            return iResult.values[0].name === tNotificationComponentName;
          });
          if (wasGraph) {
            this.handleAccomplishment('ChangeGraphTitle');
          };
      ...
      */
      ...
      //Now check the operation and the resource type against those specified in the task description under consideration

      //let tTask = taskDescriptions.descriptions.find(function (iDescription) {

      //Return true if both the operation and the component type match the values specified in the task description

      //return iDescription.operation === iNotification.values.operation &&
      //     (!iDescription.componentType || iDescription.componentType === tComponentType )
      ...
      //if( iNotification.values.operation === 'titleChange')
      ...
      codapInterface.sendRequest({
        ...
        if (iResult.success && iResult.values.length > 1) {
          iResult.values.forEach(function (iComponent) {
            if (iComponent.type === 'graph') {
              if (iComponent.name === iNotification.resource) {
              graphTitleChanged = true;
              this.handleAccomplishment('ChangeGraphTitle')
              };
            }
          });
      ...
      handleLegendAttributeChange = function () {
        if (iNotification.values.type === 'DG.GraphModel' && iNotification.values.attributeName === 'Urban Living') this.handleAccomplishment('MakeLegend');
      }.bind(this),
